...
async def create_recommendation(
    ...
    if settings.persistence_mode == "local":
        raise UnsupportedPersistenceModeError(
            "Saving recommendation runs is disabled in local persistence mode. Use /api/v1/recommendations/preview instead."
        )
...
async def get_recommendation(
    ...
    if settings.persistence_mode == "local":
        raise UnsupportedPersistenceModeError(
            "Saved recommendation runs are unavailable in local persistence mode."
        )
...
async def revert_recommendation(
    ...
    if settings.persistence_mode == "local":
        raise UnsupportedPersistenceModeError(
            "Server-side revert is unavailable in local persistence mode."
        )
