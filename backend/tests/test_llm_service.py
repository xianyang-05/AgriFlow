...
def test_llm_service_retries_with_supported_model_when_configured_model_is_missing(monkeypatch):
    logger = FakeLogger()
    monkeypatch.setattr(
        "app.services.llm_service.get_settings",
        lambda: SimpleNamespace(
    ...
    class FallbackClient:
        ...
        def post(self, url, json):
            ...
            if json["model"] == "llama3":
                return httpx.Response(404, request=request, json={"error": "model 'llama3' not found"})
            return httpx.Response(200, request=request, json={"message": {"content": "fallback ok"}})
    ...
    assert result == "fallback ok"
    assert requested_models == ["llama3", "llama3.2"]
    assert logger.warnings
    event, payload = logger.warnings[0]
    assert event == "ollama.model_fallback"
    assert payload["fallback_model"] == "llama3.2"
